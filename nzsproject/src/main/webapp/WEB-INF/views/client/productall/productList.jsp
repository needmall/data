...
<link rel="stylesheet" type="text/css"
	href="/resources/include/css/productall.css" />
...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#search").click(function() {
			if(!chkData("#address", "매장 주소를"))return;
			else {
				/* 주소 검색 */
	...
	function addItem(lat, lon) {
		...
		$.getJSON(url, function(data) {
			...
			// 검색 결과 없음
			if (!data.length) {
				$(".periphery_list").addClass("align-center");
				$(".periphery_list").html("등록된 상품이 존재하지 않습니다.");
			} else {
				$(".periphery_list").removeClass("align-center");
			}
	...
	/* Text 동적 생성 */
	function addText(locationStatus) {
		
		$(".titleStores").children("span").remove();
		if (locationStatus == 1) {
			$(".titleStores").html("주변 매장 상품&nbsp;&nbsp;");
			var addSpan = $("<span>");
			addSpan.html("(반경 : 3km)");
			$(".titleStores").append(addSpan);
		} else if (locationStatus == 0) {
			$(".titleStores").html("모든 매장 상품");
		}
	}
...
<div class="form-inline">
	<div class="form-group">
		<label for="yourlocation"> 검색된 지역 </label> <input class="form-control"
			type="text" id="yourlocation" readonly="readonly">
	</div>
	<div class="form-group">
		<label for="address">매장 검색 지역 </label> <input class="form-control"
			type="text" id="address" name="address"
			placeholder="검색할 지역을 입력하세요(예, 성동구 도선동)"> <input
			class="btn btn-default" type="button" id="search" value="검 색">
	</div>
</div>
<div>
	<form id="detailForm">
		<input type="hidden" id="ps_num" name="ps_num" /> <input
			type="hidden" id="review" name="review" />
	</form>
</div>
<div class="container restaurant-list">
										...
										<tr data-num="${FavList.ps_num}">
											...
											<td>
												<div class="restaurants-info align-center sizeWith58">
													<span class="restaurant-name ng-binding">남은수량</span> <span>${FavList.ps_count}개</span>
												</div>
											</td>
											<td class="jb-th-2">
												<div>
	...
	<div class="main_prodlist main_prodlist_list periphery">
		<h4 class="titleStores">
			<span class="titleStreet"></span>
		</h4>
		<div class="periphery_list"></div>
	</div>
</div>